    ...
    <div className="app">
      ...
      <main>
        ...
        <section className="discover" id="discover">
          ...
          <section className="validation-collection" aria-label="Historical works awaiting validation"><div className="demo-heading"><div className="eyebrow">DISCOVER THE UNFAMILIAR · REGIONAL VOICES</div><h3>Beyond Tagore: more voices to explore</h3><p>Explore real works beyond the usual recommendations. Each card offers a spoiler-light reason to explore it. Reading availability is shown separately: source-backed entries open in the app, while other editions and translations are still being validated.</p></div><div className="story-grid">{validationCandidates.map(item => <article className="story-card" key={item.id}><div className="story-card__body"><div className="eyebrow">{item.language} · {item.kind}</div><h3>{item.title}</h3><p><strong>{item.author}</strong></p><p>{item.date}</p>{item.hook && <p><strong>Why explore:</strong> {item.hook}</p>}<div className="candidate-tags"><span>{item.readerReady ? 'SOURCE TEXT · VERIFY CHAPTERS' : 'DISCOVERY · VALIDATION PENDING'}</span><span>{item.readerReady ? 'Historical text available' : 'Text not yet verified'}</span></div><p className="muted">{item.status}</p><div className="story-card__footer"><span>{item.readerReady ? "Source-backed edition" : "Source verification pending"}</span>{item.readerReady ? <button onClick={() => openHistorical(item)}>Read source text <ArrowRight size={17}/></button> : <span>Reader pending source</span>}</div></div></article>)}</div></section>
