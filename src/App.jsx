...
const validationCandidates = [
  {id:'kalikala-vaibhavam',title:'Kalikala Vaibhavam',author:'Moorkoth Kumaran',language:'Malayalam',date:'1896 · reported',kind:'Prose work',hook:'A chance to explore early Malayalam prose beyond the familiar canon.',status:'Bibliographic details and complete text need validation'},
  ...
  {id:'prabhat-wiles',title:'The Wiles of a Pleader',author:'Prabhat Kumar Mukhopadhyay',language:'Bengali',date:'Stories of Bengalee Life · 1912',kind:'Short story',status:'Published English translation; original Bengali alignment pending',sourcePage:'Stories_of_Bengalee_Life/The_Wiles_of_a_Pleader',sourceDomain:'en.wikisource.org',readerReady:true,chaptered:true,chapterCount:5,translator:'Prabhat Kumar Mukhopadhyay'},
  {id:'prabhat-release',title:'His Release',author:'Prabhat Kumar Mukhopadhyay',language:'Bengali',date:'Stories of Bengalee Life · 1912',kind:'Short story',status:'Published English translation; original Bengali alignment pending',sourcePage:'Stories_of_Bengalee_Life/His_Release',sourceDomain:'en.wikisource.org',readerReady:true,chaptered:true,chapterCount:8,translator:'Prabhat Kumar Mukhopadhyay'},
  {id:'prabhat-forest',title:'The Forest Child',author:'Prabhat Kumar Mukhopadhyay',language:'Bengali',date:'Stories of Bengalee Life · 1912',kind:'Short story',status:'Published English translation; original Bengali alignment pending',sourcePage:'Stories_of_Bengalee_Life/The_Forest_Child',sourceDomain:'en.wikisource.org',readerReady:true,chaptered:true,chapterCount:3,translator:'See source edition'},
    ...
    <div className="app">
      ...
      <main>
        ...
        <section className="discover" id="discover">
          ...
          <div className="demo-heading"><div className="eyebrow">EXPLORE THE COLLECTION · ${validationCandidates.length + historicalSelections.length + 1} WORKS</div><h3>Choose a story that catches your interest</h3><p>Real historical works across regional languages. Available source texts open in the reader; other works are clearly marked as awaiting source validation.</p></div>
          <div className="story-grid">
            <article className="story-card"><div className="story-card__body"><div className="eyebrow">Malayalam · Historical short story</div><h3>ദ്വാരക · Dwaraka</h3><p><strong>Vengayil Kunhiraman Nayanar</strong> · 1893</p><p>Explore an early Malayalam prose work.</p><div className="candidate-tags"><span>Original Malayalam · source reader</span><span>English pending</span></div><div className="story-card__footer"><span>Source text</span><button onClick={() => openHistorical()}>Explore story <ArrowRight size={17}/></button></div></div></article>
            {historicalSelections.map(item => <article className="story-card" key={item.id}><div className="story-card__body"><div className="eyebrow">Bengali · Historical English translation</div><h3>{item.title}</h3><p><strong>{item.author}</strong> · {item.edition}</p><p>{item.description}</p><div className="candidate-tags"><span>Source text · verify edition</span></div><div className="story-card__footer"><span>English text</span><button onClick={() => openHistorical(item)}>Explore story <ArrowRight size={17}/></button></div></div></article>)}
            {validationCandidates.map(item => <article className="story-card" key={item.id}><div className="story-card__body"><div className="eyebrow">{item.language} · {item.kind}</div><h3>{item.title}</h3><p><strong>{item.author}</strong> · {item.date}</p>{item.hook && <p><strong>Why explore:</strong> {item.hook}</p>}<div className="candidate-tags"><span>{item.readerReady ? 'Source reader · verify chapters' : 'Source validation pending'}</span></div><p className="muted">{item.status}</p><div className="story-card__footer"><span>{item.readerReady ? 'Historical English text' : 'Discovery only'}</span>{item.readerReady ? <button onClick={() => openHistorical(item)}>Explore story <ArrowRight size={17}/></button> : <span>Reader pending</span>}</div></div></article>)}
          </div>
