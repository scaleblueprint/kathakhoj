...
function App() {
  ...
  const [creatorSeriesId, setCreatorSeriesId] = useState('auto')
  ...
  if (creatorPage) { const current = creatorSeries.find(s => s.id === creatorSeriesId) || creatorSeries[0]; return <div className="app"><header className="topbar"><button className="brand brand--button" onClick={() => setCreatorPage(false)}>Katha<span>Khoj</span></button><button className="ghost" onClick={() => setCreatorPage(false)}><ArrowLeft size={18}/> Back to Discover</button></header><main className="story-page"><section className="story-hero"><div><div className="eyebrow">KATHAKHOJ · CREATOR STUDIO</div><h1>Stories waiting to be told.</h1><p>Original fiction concepts inspired by regional literary discovery and Praveen's reflective, conversational blog voice. These are new story ideas, not adaptations or unpublished works by historical authors.</p><p><strong>Editorial voice:</strong> Begin with an ordinary incident; notice a small human detail; let humour and memory lead into a thoughtful question. Keep the narrator curious, warm and personal rather than preachy.</p><a href="https://praveenvs25.blogspot.com/" target="_blank" rel="noopener noreferrer">Writing reference: My incomplete story ↗</a></div><div className="visual visual--green visual--hero"><div className="visual__glyph">✦</div><div className="visual__region">Creator Studio</div><div className="visual__caption">IDEA · EPISODES · MULTI-CHANNEL</div></div></section><section className="discover"><div className="section-head"><div><div className="eyebrow">ORIGINAL SERIES CONCEPTS</div><h2>Choose a story to develop</h2></div><p>Four original concepts with episodic arcs and channel-specific formats. Draft concepts, not completed scripts.</p></div><div className="story-grid">{creatorSeries.map(s => <article className="story-card" key={s.id}><div className="story-card__body"><div className="eyebrow">{s.format}</div><h3>{s.title}</h3><p><strong>Creative starting point:</strong> {s.inspiration}</p><p><strong>Why explore:</strong> {s.premise}</p><div className="story-card__footer"><span>{s.episodes.length} proposed episodes</span><button onClick={() => {setCreatorSeriesId(s.id); document.getElementById('creator-detail')?.scrollIntoView({behavior:'smooth'})}}>View series <ArrowRight size={17}/></button></div></div></article>)}</div></section><section className="discover" id="creator-detail"><div className="demo-heading"><div className="eyebrow">SERIES DEVELOPMENT · {current.format}</div><h2>{current.title}</h2><p>{current.premise}</p><p><strong>Reflective question:</strong> {current.question}</p></div><div className="story-grid"><article className="story-card"><div className="story-card__body"><h3>Episode map</h3><ol>{current.episodes.map((e,i)=><li key={e}>Episode {i+1}: {e}</li>)}</ol><p className="muted">Story beats and scripts are development drafts; no historical source is being represented as original fiction.</p></div></article><article className="story-card"><div className="story-card__body"><h3>Channel adaptations</h3>{current.channels.map(c=><p key={c}>• {c}</p>)}<p><strong>Repeatable episode structure:</strong> personal hook → scene → unexpected human detail → turning point → reflective closing question.</p></div></article></div></section></main></div> }
